...
// Logica compartida por el servidor Express de desarrollo (server/index.js) y por
// la funcion serverless de produccion (netlify/functions/contact.js).
...
async function handleContact(body, options = {}) {
  ...
  if (!payload.ok) {
    return { status: 400, body: { ok: false, message: payload.message } };
  }
  ...
  // La cuenta de prueba Ethereal solo tiene sentido en desarrollo: el llamador
  // decide si esta permitida, produccion nunca debe caer en ella en silencio.
  const allowTestAccount =
    options.allowTestAccount === true &&
    process.env.NODE_ENV !== 'production' &&
    process.env.SMTP_ALLOW_TEST_ACCOUNT !== 'false';
  ...
  if (missingConfig.length === 0) {
    transporter = nodemailer.createTransport({
      ...
      // Sin estos limites, un puerto SMTP bloqueado por el hosting deja la
      // peticion colgada varios minutos y el proxy acaba devolviendo un 504.
      ...timeouts,
    });
  } else if (allowTestAccount) {
    ...
    console.error(`[contact-api] Configuracion SMTP incompleta. Faltan: ${missingConfig.join(', ')}`);
    return {
      status: 500,
      body: {
        ok: false,
  ...
  try {
    ...
    if (error && error.code === 'EAUTH') {
      console.error(
        '[contact-api] Gmail exige una contrasena de aplicacion; la contrasena normal de la cuenta no sirve.',
      );
      return {
        status: 502,
        body: { ok: false, message: 'No se pudo enviar la consulta en este momento.' },
      };
    }

    if (error && (error.code === 'ETIMEDOUT' || error.code === 'ESOCKET' || error.code === 'ECONNECTION')) {
      console.error(
        `[contact-api] Sin salida SMTP hacia ${smtpHost}:${smtpPort}. Algunos hostings (Render free entre ellos) bloquean los puertos 25/465/587.`,
      );
      return {
        status: 502,
        body: { ok: false, message: 'No se pudo contactar con el servidor de correo.' },
      };
    }
...
function normalizePayload(body) {
  ...
  return {
    ok: true,
    value: { course, fullName, email, phone, message },
  };
}
...
module.exports = { handleContact, normalizePayload, escapeHtml, parseBoolean };
